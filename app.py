...
# Small CSS tweaks for a cleaner look + dark theme
st.markdown(
    "<style>"
    "body, .stApp, .main, .block-container { background-color: #000000 !important; color: #ffffff !important; }"
    "[data-testid='stSidebar'] { background-color: #0b0b0b !important; color: #e6e6e6 !important;}"
    ".st-bx { color: #ffffff !important; }"
...
# Replace numeric inputs with sliders for better UX
# Annual income slider: 0 to 5,000,000 with a step of 10,000 and default 500,000
annual_income = st.sidebar.slider('Annual Income (₹)', 0, 5000000, 500000, step=10000)
# Loan amount slider: 0 to 2,000,000 with a step of 5,000 and default 200,000
...
# Assets slider: 0 to 5,000,000 with a step of 10,000 and default 100,000
assets = st.sidebar.slider('Assets Annual Income (₹)', 0, 5000000, 100000, step=10000)

# Encode categorical variables
grad_s = 0 if grad == 'Graduated' else 1
emp_s = 0 if self_emp == 'No' else 1

# Top-level metrics area (show live values)
...
if predict_button:
    ...
    # Display result and probability if available
    if getattr(model, 'predict_proba', None) is not None:
        prob = model.predict_proba(pred_scaled)[0]
        # assume class 1 is approval; try to find which column corresponds to approval
    ...
    finance_data = pd.DataFrame({
        'Parameter': ['Annual Income', 'Loan Amount', 'Loan Duration', 'CIBIL Score', 'Assets'],
    ...
    # ------------------------------
    # Loan Term Impact (EMI vs Loan Duration)
    st.markdown("---")
    st.subheader("Loan Term Impact on EMI")
    interest_rate = 0.10  # Example annual interest rate 10%
    months = list(range(1, loan_dur*12 + 1))
    emi = [loan_amount * (interest_rate/12) / (1 - (1 + interest_rate/12)**(-m)) for m in months]
    emi_df = pd.DataFrame({'Month': months, 'EMI': emi})
    fig3 = px.line(emi_df, x='Month', y='EMI', title='EMI vs Loan Duration (Months)')
    st.plotly_chart(fig3, use_container_width=True)
    
    # ------------------------------
    # CIBIL Score Visualization (Gauge)
    st.markdown("---")
